...
const DataManagement = () => {
  ...
  const deleteTransaction = (id: number) => {
    ...
    if (transaction) {
      ...
      toast({
        ...
        description: `${transaction.name} on poistettu`,
      });
    }
  };
  ...
  const deleteLoan = (id: number) => {
    ...
    if (loan) {
      ...
      toast({
        ...
        description: `${loan.name} on poistettu`,
      });
    }
  };
  ...
  const deleteMonthlyBill = (id: number) => {
    ...
    if (bill) {
      ...
      toast({
        title: "Kuukausimaksu poistettu",
        description: `${bill.name} on poistettu`,
      });
    }
  };

  const handleClearAllData = () => {
    if (confirm("Haluatko varmasti poistaa kaikki tiedot? Tätä toimintoa ei voi peruuttaa.")) {
      clearAllData();
      refreshData();
      toast({
        title: "Kaikki tiedot poistettu",
        description: "Sovellus on nollattu",
      });
    }
  };
  ...
  return (
    <div className="p-4 pb-20 bg-[#192E45] min-h-screen">
      ...
      <div className="flex items-center space-x-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-white">Tiedonhallinta</h1>
      </div>

      {/* Transactions */}
      <Card className="mb-6 bg-[#294D73] border-none">
        <CardHeader>
          <CardTitle className="text-white">Tapahtumat ({financialData.transactions.length})</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {financialData.transactions.length === 0 ? (
            <div className="text-white/70 text-center py-4">Ei tapahtumia</div>
          ) : (
      ...
      <Card className="mb-6 bg-[#294D73] border-none">
        <CardHeader>
          <CardTitle className="text-white">Lainat ({financialData.loans.length})</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {financialData.loans.length === 0 ? (
            <div className="text-white/70 text-center py-4">Ei lainoja</div>
          ) : (
            financialData.loans.map(loan => (
              <div key={loan.id} className="flex items-center justify-between p-3 bg-white/10 rounded-lg">
                <div>
                  ...
                  <div className="text-sm text-white/70">€{loan.currentAmount} • €{loan.monthly}/kk</div>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
      ...
      <Card className="mb-6 bg-[#294D73] border-none">
        <CardHeader>
          <CardTitle className="text-white">Kuukausimaksut ({financialData.monthlyBills.length})</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {financialData.monthlyBills.length === 0 ? (
            <div className="text-white/70 text-center py-4">Ei kuukausimaksuja</div>
          ) : (
